...
import appeng.core.me.grid.storage.MEMonitorPassThrough;
import appeng.core.me.grid.storage.NullInventory;
import appeng.core.me.part.automation.StackUpgradeInventory;
...
public class DualityInterface implements IGridTickable, IStorageMonitorable, IInventoryDestination, IAEAppEngInventory, IConfigManagerHost, ICraftingProvider, IUpgradeableHost, IPriorityHost
{
	...
	private void updatePlan( final int slot )
	{
		...
		if( req == null && Stored != null )
		{
			...
			this.requireWork[slot] = work.setCount( -work.getCount() );
			return;
		}
		else if( req != null )
		{
			...
			{
				if( req.getCount() != Stored.getCount() )
				{
					this.requireWork[slot] = req.copy();
					this.requireWork[slot].setCount( req.getCount() - Stored.getCount() );
					return;
				}
			}
			else
	...
	private boolean usePlan( final int x, final IAEItemStack itemStack )
	{
		...
		try
		{
			...
			else if( itemStack.getCount() < 0 )
			{
				...
				toStore.setCount( -toStore.getCount() );

				long diff = toStore.getCount();
	...
	private boolean handleCrafting( final int x, final InventoryAdaptor d, final IAEItemStack itemStack )
	{
		try
		{
			...
			{
				return this.craftingTracker.handleCrafting( x, itemStack.getCount(), itemStack, d, this.iHost.getTileEntity().getWorld(), this.gridProxy.getGrid(), this.gridProxy.getCrafting(), this.mySource );
			}
		}
		catch( final GridAccessException e )
		{
