...
public class MEItemDefinitions extends Definitions<Item, IItemDefinition<Item>> implements IMEItemDefinitions
{
	...
	{
		this.cellCreative = registry.item( "creative_storage_cell", new ItemCreativeStorageCell() ).features( AEFeature.StorageCells, AEFeature.Creative ).build();
		this.viewCell = registry.item( "view_cell", new ItemViewCell() ).build();
