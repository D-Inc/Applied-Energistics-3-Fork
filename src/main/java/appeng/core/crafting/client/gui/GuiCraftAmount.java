...
public class GuiCraftAmount extends AEBaseGui
{
	...
	public void initGui()
	{
		...
		{
			myIcon = AppEngME.INSTANCE.<Item, MEItemDefinitions>definitions( Item.class ).wirelessTerminal().maybeStack( 1 ).orElse( myIcon );
		...
		{
			myIcon = parts.terminal().maybeItem().get().maybeStack( 1 ).orElse( null );
			this.originalGui = GuiBridge.GUI_ME;
		}

		if( target instanceof PartCraftingTerminal )
		{
			myIcon = parts.craftingTerminal().maybeItem().get().maybeStack( 1 ).orElse( null );
			this.originalGui = GuiBridge.GUI_CRAFTING_TERMINAL;
		}

		if( target instanceof PartPatternTerminal )
		{
			myIcon = ( (CraftingPartDefinitions<?>) (CraftingPartDefinitions) AppEngCrafting.INSTANCE.definitions( PartRegistryEntry.class ) ).patternTerminal().maybeItem().get().maybeStack( 1 ).orElse( null );
